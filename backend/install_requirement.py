...
def install_with_pip(env):
    if env == "install-dev":
        subprocess.call(['pipenv', 'lock', '--requirements', '--dev'], stdout=open('dev-requirements.txt', 'w'))
        subprocess.call(['pip', 'install', '-r', 'dev-requirements.txt'])
    else:
        subprocess.call(['pipenv', 'lock', '--requirements'], stdout=open('requirements.txt', 'w'))
        subprocess.call(['pip', 'install', '-r', 'requirements.txt'])

if __name__ == "__main__":
    if len(sys.argv) != 3:
        print("Usage: python install_requirements.py [pip|pipenv] [install|install-dev]")
        sys.exit(1)
