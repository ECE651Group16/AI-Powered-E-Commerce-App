...
if __name__ == "__main__":
    ...
    asin_code, asin_uniques = pd.factorize(
        pd.concat([meta_table["parent_asin"], table["parent_asin"]])
    )
    ...
    with MySQLdb.connect(
        ...
        for i in tqdm(range(user_limit)):
            ...
            cur.execute(
                ...
                (id, phone, birth_date, membership, user_id),
            )
        ...
        for i in tqdm(range(len(limit_table))):
            ...
            cur.execute(
                "INSERT INTO store_order(id, placed_at, payment_status, customer_id) VALUES(%s,%s,%s,%s)",
