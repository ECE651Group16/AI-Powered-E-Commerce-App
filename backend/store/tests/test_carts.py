...
class TestCreateCart:
    ...
    def test_add_product_to_cart_if_data_invalid(self, api_client, create_cart):
        ...
        response = api_client.post(
            f"/store/carts/{cart_id}/items/", {"product_id": product.id, "quantity": -1}
        )
    ...
    def test_add_product_to_cart_if_data_valid(self, api_client, create_cart):
        ...
        response_item_added = api_client.post(
            f"/store/carts/{cart_id}/items/", {"product_id": product.id, "quantity": 10}
        )
    ...
    def test_delete_item_from_cart(self, api_client, create_cart):
        ...
        cart_item_id = find_cart_item_id_from_product_id(
            response_item_added.data, product.id
        )
        response_item_deleted = api_client.delete(
            f"/store/carts/{cart_id}/items/{cart_item_id}/"
        )
