...
class LikedItem(models.Model):
    likes = models.ForeignKey(
        Likes, on_delete=models.CASCADE, related_name="items"
    )  # cartitem_set
    ...
    def add_to_cart(self):
        ...
        if customer is None:
            ...
            raise ValidationError(
                "This liked item has no associated customer to add to a cart."
            )
        cart, created = Cart.objects.get_or_create(customer=customer)

        # Retrieve or create a CartItem for the product
        cart_item, created = CartItem.objects.get_or_create(
            cart=cart, product=self.product, defaults={"quantity": 1}
        )
