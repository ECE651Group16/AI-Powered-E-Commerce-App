...
class LikesViewSet(CreateModelMixin, 
                  RetrieveModelMixin,
                  DestroyModelMixin,
                  GenericViewSet):
    ...
    def create(self, request, *args, **kwargs):
        ...
        if Likes.objects.filter(customer=user.customer).exists():
            return Response({'detail': 'Likes already exists for this user.'}, status=status.HTTP_400_BAD_REQUEST)
...
class LikedItemViewSet(ModelViewSet):
    ...
    def get_queryset(self):
        return LikedItem.objects \
            .filter(likes_id=self.kwargs['likes_pk']) \
